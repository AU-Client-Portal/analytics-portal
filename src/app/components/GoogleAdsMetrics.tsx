...
// Staggered entry card with pulse-on-hover
function AdMetricCard({ title, value, sub, icon, index, accentColor, t }: {
  ...
  return (
    <div
      ...
      className={`${t.card} border ${t.border} rounded-2xl p-5 cursor-default group`}
    >
      <div className="flex items-start justify-between mb-3">
        <span className={`${t.subtext} text-xs font-semibold uppercase tracking-widest`}>{title}</span>
        <span style={{ color: accentColor, opacity: hovered ? 1 : 0.45, transition: 'opacity 0.2s, transform 0.3s', transform: hovered ? 'scale(1.2) rotate(-6deg)' : 'scale(1)' }}>
          {icon}
        </span>
      </div>
...
// Animated horizontal bar row for campaign table
    ...
    >
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-4 shrink-0 ml-3">
          <span style={{ color: colors.ring2, fontSize: 12, fontWeight: 700 }}>${campaign.cost.toFixed(0)}</span>
          <span style={{ color: colors.ring1, fontSize: 12, fontWeight: 700 }}>{campaign.conversions} conv.</span>
          <span className={`${t.subtext} text-xs`}>ROAS {roas}×</span>
        </div>
      </div>
      <div className="space-y-1.5">
        <div style={{ height: 5, background: colors.track, borderRadius: 3, overflow: 'hidden' }}>
          <div style={{
            height: '100%', width: visible ? `${costPct}%` : '0%',
            background: colors.ring2, borderRadius: 3,
        ...
        <div style={{ height: 5, background: colors.track, borderRadius: 3, overflow: 'hidden' }}>
          <div style={{
            height: '100%', width: visible ? `${convPct}%` : '0%',
            background: colors.ring1, borderRadius: 3,
            transition: 'width 0.9s cubic-bezier(0.4,0,0.2,1)',
      ...
      <div className="flex gap-4 mt-1.5">
        <span className={`${t.subtext} text-xs`} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
...
const CustomTooltip = ({ active, payload, label, t }: any) => {
  ...
  return (
    <div className={`${t.card} border ${t.border} rounded-xl p-3 shadow-xl text-sm`}
      ...
      {payload.map((p: any, i: number) => (
        <p key={i} style={{ color: p.color }} className="font-semibold">
          {p.name}: {typeof p.value === 'number' && p.name.includes('$')
            ? `$${p.value.toFixed(2)}` : p.value?.toLocaleString()}
        </p>
      ))}
    </div>
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <AdMetricCard title="Impressions"  value={m.impressions.toLocaleString()}              icon={<Eye size={16}/>}              index={0} accentColor={cardAccents[0]} t={t} />
        <AdMetricCard title="Clicks"       value={m.clicks.toLocaleString()}                   icon={<MousePointerClick size={16}/>} index={1} accentColor={cardAccents[1]} t={t} />
        <AdMetricCard title="CTR"          value={`${m.ctr.toFixed(2)}%`}                      icon={<Percent size={16}/>}           index={2} accentColor={cardAccents[2]} t={t} />
        <AdMetricCard title="Total Spend"  value={`$${m.cost.toLocaleString('en-US', { minimumFractionDigits: 2 })}`}  icon={<DollarSign size={16}/>}        index={3} accentColor={cardAccents[3]} t={t} />
        <AdMetricCard title="Conversions"  value={m.conversions.toFixed(0)}                    icon={<Target size={16}/>}            index={4} accentColor={cardAccents[4]} t={t} />
        <AdMetricCard title="Conv. Value"  value={`$${m.conversionsValue.toLocaleString('en-US', { minimumFractionDigits: 2 })}`} icon={<Award size={16}/>} index={5} accentColor={cardAccents[5]} t={t} />
        <AdMetricCard title="Avg CPC"      value={`$${m.averageCpc.toFixed(2)}`}               icon={<TrendingUp size={16}/>}        index={6} accentColor={cardAccents[6]} t={t} />
        <AdMetricCard title="Cost / Conv." value={m.conversions > 0 ? `$${(m.cost / m.conversions).toFixed(2)}` : '—'} sub={`${m.conversions > 0 ? ((m.conversionsValue / m.cost).toFixed(2)) : '—'}× ROAS`} icon={<Calculator size={16}/>} index={7} accentColor={cardAccents[7]} t={t} />
      </div>
      ...
      {campaigns.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-4 text-xs" style={{ color: t.subtext }}>
