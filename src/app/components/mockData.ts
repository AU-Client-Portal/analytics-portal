...
const randF = (lo: number, hi: number) => +(lo + Math.random() * (hi - lo)).toFixed(2);
...
const ga4TimeSeries = Array.from({ length: 90 }, (_, i) => {
  ...
  const phoneCallClicks    = Math.floor(activeUsers * (0.018 + Math.random() * 0.008));
  ...
  const callButtonClicks   = Math.floor(activeUsers * (0.025 + Math.random() * 0.01));
  const callConnected      = Math.floor(phoneCallClicks * (0.58 + Math.random() * 0.12));
  return {
    date: yyyymmdd(90 - i),
    activeUsers, sessions, pageViews, bounceRate, engagementRate,
    newUsers, avgSessionDuration,
    phoneCallClicks, callButtonClicks, callConnected,
    formSubmits, contactPageViews, quoteRequests,
  };
});

const adsTimeSeries = Array.from({ length: 90 }, (_, i) => {
  const isWeekend = i % 7 === 5 || i % 7 === 6;
  const wm = isWeekend ? 0.7 : 1;
  const gt = 1 + (i / 90) * 0.3;
  const base = 3000 * gt * wm;
  const impressions   = Math.floor(base + rand(-300, 300));
  const clicks        = Math.floor(impressions * (0.038 + Math.random() * 0.008));
  const cost          = +(clicks * (1.18 + Math.random() * 0.16)).toFixed(2);
  const conversions   = Math.floor(clicks * (0.055 + Math.random() * 0.015));
  const convValue     = +(conversions * (78 + rand(-20, 20))).toFixed(2);
  return { date: yyyymmdd(90 - i), impressions, clicks, cost, conversions, convValue };
});
...
const socialTimeSeries = Array.from({ length: 90 }, (_, i) => {
  ...
  return { date: yyyymmdd(90 - i), reach, impressions, engagement, followers };
});
...
const gscTimeSeries = Array.from({ length: 90 }, (_, i) => {
  ...
  return { date: yyyymmdd(90 - i), clicks, impressions, ctr, position };
});
...
export const MOCK_SEARCH_CONSOLE = {
  companyId:   'company-001',
  companyName: 'Acme Digital Co.',
  siteUrl:     'https://acmedigital.com',
  dateRange: { startDate: yyyymmdd(90), endDate: yyyymmdd(0) },
  metrics: {
    totalClicks:      47_210,
